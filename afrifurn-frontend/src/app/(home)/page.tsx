...
export default function HomePage() {
  return (
    // Wraap with Suspense
      ...
      {/* Hero Carousel */}
      <section className="relative h-[90vh] md:h-[85vh]">
        <Carousel className="w-full h-full">
          <CarouselContent className="h-full">
            {carouselItems.map((item, index) => (
              <CarouselItem key={index} className="h-full">
                <div className="relative h-[90vh]">
                  <AspectRatio ratio={16/9} className="hidden md:block h-[90vh]">
                    <Image
                      src={item.image}
                      alt={item.title}
                      fill
                  ...
                  <div className="absolute inset-0 flex items-center justify-start bg-black/10">
                    <div className="text-left text-black max-w-xl px-8 md:px-16 py-8 bg-white/80 ml-0 md:ml-16 transform transition-all duration-700">
                      <h2 className="text-2xl md:text-4xl font-bold mb-2 md:mb-4">{item.title}</h2>
